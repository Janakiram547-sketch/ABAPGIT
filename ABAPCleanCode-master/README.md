...
**This repository is deprecated. Use [_Clean ABAP_ on github.com](https://github.com/SAP/styleguides/blob/master/clean-abap/CleanABAP.md) instead.**

On May 3rd, 2019, we published a copy of this repository as open-source on github.com,
to enable the full ecosystem around ABAP to benefit and participate. 
Since then, the public variant has slowly but determinedly evolved,
while this repository has not received any more updates.

Unfortunately, there is no easy way to fork from the public github.com
to our SAP-internal github.wdf.sap.corp.
Also, the structure of the two repositories is different,
as the public one was designed to host other style guides in the future.
As a consequence, it is too error-prone and work-intensive
to keep the two synced through manual activity.
 
You can still use this repository to start discussions that should not become public,
and to keep older links alive; we will keep watching it and won't delete it.
However, kindly visit the open source variant for up-to-date information.
Please also don't be disappointed if we add your contributions and discussion results only there.
...
The [Cheat Sheet](CheatSheet.md) is a print-optimized version.
...
Consider resolving complex constructions to a multi-step construction with the [design pattern _builder_](https://en.wikipedia.org/wiki/Builder_pattern). 
...
`RETURNING` not only makes the call shorter, it also allows method chaining and prevents [same-input-and-output errors](#take-care-if-input-and-output-could-be-the-same). 
...
> This section contradicts the ABAP Programming Guidelines and Code Inspector checks, both of whom suggest that large tables should be EXPORTED by reference to avoid performance deficits.
We consistently failed to reproduce any performance and memory deficits.
[Issue 22](https://github.wdf.sap.corp/CleanCode/ABAPCleanCode/issues/22) suggests that there is indeed kernel optimization to improve RETURNING performance and that this guideline therefore is okay.
...
> [Clean ABAP](#clean-abap) > [Content](#content) > [Methods](#methods) > [Parameter Initialization](#parameter-initialization) > [Take care if input and output could be the same](#take-care-if-input-and-output-could-be-the-same)
...
> This example has been challenged from a performance point of view, claiming that cutting the methods so small worsens performance too much.
[Measurements](https://ldcifri.wdf.sap.corp:44300/sap/fra/reverb?show-me=zcl_express_yourself_in_code&method=measure_and_compare) show that the refactored code is 2.13 times slower than the original dirty variant.
The clean variant takes 9.6 microseconds to fix 31-02-2018, the dirty variant only 4.5 microseconds.
This may be a problem when the method is run very often in a high-performance application.
For regular cases, it may not be an issue.
...
> If you modify existing unformatted code, you might want to apply the pretty printer only to the changed code to avoid additional changes in the code, which would hide the functional changes. 
